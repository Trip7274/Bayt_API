...
public static class Permissions
{
	...
	*/
	public sealed class BaytPermission : IAuthorizationRequirement
	{
		public BaytPermission(string permissionString, List<string>? permPowers = null)
		{
		...
		public BaytPermission(string permissionString, string? permPowers = null)
		{
			if (string.IsNullOrWhiteSpace(permissionString)) throw new ArgumentException("Permission string cannot be empty", nameof(permissionString));
		...
		public static bool TryParse(string constructedPerm, [NotNullWhen(true)] out BaytPermission? permReq)
		{
			...
			{
				permReq = new BaytPermission(permissionString, splitString[1]);
				return true;
			}
			var permPowers = splitString[1].Split(',').Select(p => p.Trim()).ToList();


			permReq = new BaytPermission(permissionString, permPowers);
			return true;
		}
		...
		public static implicit operator string(BaytPermission perm) => perm.ToString();
	}

	public sealed class PermissionHandler : AuthorizationHandler<BaytPermission>
	{
		protected override Task HandleRequirementAsync(AuthorizationHandlerContext context, BaytPermission requirement)
		{
		...
		: DefaultAuthorizationPolicyProvider(options)
	{
		...
		{
			if (!BaytPermission.TryParse(policyName, out var requirement))
	...
	public sealed class BaytAuthorizationMessageHandler : IAuthorizationMiddlewareResultHandler
	{
