	...
	public static void MapUserSecurityEndpoints(this IEndpointRouteBuilder app)
	{
		...
		app.MapPatch($"{BaseUsersUrl}/users/{{targetUserId}}/permissions", (HttpContext context, string targetUserId, [FromBody] Dictionary<string, List<string>> permissionsToAdd) =>
		{
			...
			foreach (var (permission, scopes) in permissionsToAdd)
			{
				...
				if (!connectedUser.HasPermission(baytPermission)) continue;

				targetUser.AddPermission(baytPermission);
				changesMade = true;
			}
		...
		app.MapDelete($"{BaseUsersUrl}/users/{{targetUserId}}/permissions", (HttpContext context, string targetUserId, [FromBody] Dictionary<string, List<string>> permissionsToRemove) =>
		{
			...
			{
				var baytPermission = new Permissions.BaytPermission(permission, scopes);
				if (!connectedUser.HasPermission(baytPermission) || !targetUser.PermissionList.ContainsKey(permission)) continue;

				targetUser.RemovePermission(baytPermission);
				changesMade = true;
			}
		...
		app.MapPut($"{BaseUsersUrl}/users/{{targetUserId}}/permissions", (HttpContext context, string targetUserId, [FromBody] Dictionary<string, List<string>> permissionsToSet) =>
		{
			...
			foreach (var (permission, scopes) in permissionsToSet)
			{
				...
				if (!connectedUser.HasPermission(baytPermission))
					return Results.BadRequest(new { message = "All permissions must be in the list of connected user's permissions.", missingPermission = baytPermission.ToString() });
			}
