...
public static class ShellMethods
{
	...
	/// <param name="environmentVariables">Environment variables to set for the specified process. These are applied over the Sofa API's env vars.</param>
	...
	public static async Task<ShellResult> RunShell(string program, string[]? arguments = null, TimeSpan? timeout = null, bool throwIfTimedout = true, Dictionary<string, string?>? environmentVariables = null)
	{
		...
		Logs.LogBook.Write(new LogEntry(LogStream.Verbose, $"Command Exec [{processIdentifier:X8}]", $"Got a request to run a command: '{Path.GetFileName(program)} [{string.Join(", ", arguments)}]'"));
		StringBuilder stdout = new();
		StringBuilder stderr = new();
		Dictionary<string, string?> envVars = new()
		{
			{ "SOFA_SUBPROCESS", "1" }, // In the future, also expose the requesting client and user's identifiers/names?
			{ "SOFA_SUBPROCESS_ID", processIdentifier.ToString() },
			{ "SOFA_APIVERSION", ApiConfig.ApiVersion.ToString() },
			{ "SOFA_VERSION", ApiConfig.Version }
		};
		...
		catch (OperationCanceledException e)
		{
			if (throwIfTimedout)
			{
				Logs.LogBook.Write(new(LogStream.Fatal,
					$"Command Exec [{processIdentifier:X8}]", $"The process '{Path.GetFileName(program)}' fatally timed out after {processTimer.Elapsed.TotalSeconds} seconds. Sofa will exit."));
		...
		Logs.LogBook.Write(new LogEntry(LogStream.Verbose, $"Command Exec [{processIdentifier:X8}]",
