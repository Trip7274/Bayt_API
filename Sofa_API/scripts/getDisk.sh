...
# This part of Sofa is one of the first to be handled in a "hybrid" way. This means that, if a script has a specific $STAT
# in its "SCRIPTSUPPORTS" variable (which should be echoed if the $STAT is "Meta.Supports"),
# then Sofa will execute the script for that stat, rather than the internal C# method.
# This is to maintain the customizability of Sofa, while allowing more functions to be implemented natively by default.
